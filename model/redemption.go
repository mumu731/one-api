...
// 充值增加积分余额
func Recharge(quantity float64, userId int) (msg string, err error) {
	err = DB.Transaction(func(tx *gorm.DB) error {
		err = tx.Model(&User{}).Where("id = ?", userId).Update("quota", gorm.Expr("quota + ?", quantity)).Error
		if err != nil {
			return err
		}
		return nil
	...
	if err != nil {
		return "失败", errors.New("充值失败，" + err.Error())
	}
	RecordLog(userId, LogTypeTopup, fmt.Sprintf("用户充值 %.6f", quantity))
